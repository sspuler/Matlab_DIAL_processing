...
flag.decimate = 0; % decimate data to half the wv resoltuion (keep native gate spacing)
...
if strcmp(node,'NCAR')==1
  ...
  files = uipickfiles('prompt', 'select data files to process', 'REFilter', 'NF', 'FilterSpec', '/scr/eldora1/wvdial_2_data/2017');
  ave_time.wv = 5.0; % averaging time (in minutes)  
  ave_time.rb = 1; % averaging time (in minutes) 
  ave_time.gr = 1; % gridding time (in minutes) for the output files  
  diff_geo_on = 1;
  AB_scale=0.2;
  %switch_ratio = 1;
end
...
for j = 1:size(files,2)
    ...
    if strcmp(node,'NCAR')==1
      ...
      HSRL_Analysis_v3_function(folder, MCS, write_data_folder, flag, node, ...
        profiles2ave, P0, ave_time, timing_range_correction, blank_range, ...
        p_hour, receiver_scale_factor, diff_geo_on, AB_scale, diff_geo_corr, catalog)%
    end
end